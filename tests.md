...
| `internal/engine` | `pkg_engine_test.go` | `renderNames` (plain, conditional true/false, parse error, multi-line templates), `renderCached` (no template, variable substitution, missing key, syntax error), `shellQuote` (simple, spaces, quotes, empty), `rawData`/`shellData` (data merging), multi-line packages/services conditionals |
